...
function DatePicker({ value, onChange, placeholder, onBlur, error, ref, name, id }: MonthPickerProps) {
      ...
      <div className={clsx({ [styles.calendar]: true, [styles.visible]: visible })}>
        ...
        <div className={clsx({ [styles.daysOfWeek]: true })}>
          {daysOfWeek.map((c, index) => (
            <div className={clsx({ [styles.dayOfWeek]: true, [styles.weekend]: index === 0 || index === 6 })}>{c}</div>
          ))}
          {days.map((d, index) => (
            <>
